...
_.extend(Provider.prototype, {
    ...
    addApp: function(title, description, callback) {
        this.keyPair(function(err, pair) {
            if (err) {
                callback(err, null);
            } else {
                var app = {
                ...
                this.db.save('app', app.consumer_key, app, callback);
            }
        });
    },
    keyPair: function(callback) {
        this.randomString(16, function(err, consumer_key) {
            if (err) {
                callback(err, null);
            } else {
                this.randomString(16, function(err, secret) {
                    if (err) {
                        callback(err, null); 
                    } else {
